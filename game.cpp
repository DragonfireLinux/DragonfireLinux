...
void Game::processKeyInput()
{
	...
	}else if (wasKeyDown(KeyType::MUTE))
	{
		...
		if(set == 0)
		{
			g_settings->set("enable_fastaction", bool_to_cstr(false));
			g_settings->set("enable_zeus", bool_to_cstr(true));
			g_settings->set("enable_shaders", bool_to_cstr(true));
			m_game_ui->showTranslatedStatusText("Normal Mode");
		}
		else if(set == 1)
		{
			g_settings->set("enable_fastaction", bool_to_cstr(false));
			g_settings->set("enable_shaders", bool_to_cstr(false));
			m_game_ui->showTranslatedStatusText("Bright Mode");
		}
		else if(set == 2)
		{
			g_settings->set("enable_fastaction", bool_to_cstr(false));
			g_settings->set("enable_shaders", bool_to_cstr(false));
			g_settings->set("enable_zeus", bool_to_cstr(false));
			m_game_ui->showTranslatedStatusText("Zeus Mode");
		}
		else if(set == 3)
		{
			g_settings->set("enable_fastaction", bool_to_cstr(true));
			g_settings->set("enable_shaders", bool_to_cstr(false));
			g_settings->set("enable_zeus", bool_to_cstr(true));
			m_game_ui->showTranslatedStatusText("Fast Place Mode");
		}

		// force the world to update render infront of the player
		const v3f camera_direction = camera->getDirection();
		const v3s16 camera_offset  = camera->getOffset();

		core::line3d<f32> shootline;
		shootline.start = camera->getPosition();
		shootline.end = shootline.start + camera_direction * 10000;
		...
		client->setCrack(1337, pointed.node_undersurface);

		//client->setCrack(2, client->getEnv().getLocalPlayer()->getStandingNodePos());
		//addUpdateMeshTaskForNode(client->getEnv().getLocalPlayer()->getStandingNodePos(), false, true);

		// const bool ns = !g_settings->getBool("enable_shaders");
		// g_settings->set("enable_shaders", bool_to_cstr(ns));
...
void Game::toggleFreeMove()
{
	...
	if (free_move) {
		if (client->checkPrivilege("fly")) {
			...
			m_game_ui->showTranslatedStatusText("Fly mode enabled (note: no 'fly' privilege) gosh :(");
		}
	} else {
...
void Game::toggleFast()
{
	...
	if (fast_move) {
		if (has_fast_privs) {
			...
			m_game_ui->showTranslatedStatusText("Fast mode enabled (note: no 'fast' privilege) noo :(");
		}
	} else {
...
void Game::toggleNoClip()
{
	...
	if (noclip) {
		if (client->checkPrivilege("noclip")) {
			...
			m_game_ui->showTranslatedStatusText("Noclip mode enabled (note: no 'noclip' privilege) damn :(");
		}
	} else {
...
void Game::processPlayerInteraction(f32 dtime, bool show_hud, bool show_debug)
{
	...
	*/
	...
	f32 d = getToolRange(selected_def, hand_item.getDefinition(itemdef_manager)) + 1000; // !!HAX
	...
	const ItemStack &selected_item, const ItemStack &hand_item, f32 dtime)
{
	...
	*/
	...
	if ((wasKeyPressed(KeyType::PLACE) ||
		...
		if(g_settings->getBool("enable_fastaction")) // !!HAX
			runData.repeat_place_timer = m_repeat_place_time;
		else
			runData.repeat_place_timer = 0;
...
#else
	// !!HAX
	static const std::string control_text_template = strgettext("This version is Hax0red tf.\n\nEnabled Constantly:\nno fall damage, all permissions (fly, noclip, fast mode), no slip, fast dig, extended tool range, cheat detection disabled.\n\nPress 'M' to cycle cheat modes.\n - Normal\n - Fullbright (no night)\n - Zeus (see through stone / mining xray)\n - Fast Place Blocks"
	);
